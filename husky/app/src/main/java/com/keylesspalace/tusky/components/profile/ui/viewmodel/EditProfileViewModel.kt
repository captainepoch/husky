    ...
    fun obtainProfile() {
        if (profileData.value == null || profileData.value is Error) {
            ...
            mastodonApi.accountVerifyCredentials()
                .subscribe(
                    { profile ->
                        val accountSource = profile?.source
                        with(accountSource?.fields) {
                            oldProfileData = if (this.isNullOrEmpty()) {
                                profile.copy(
                                    source = accountSource?.copy(
                        ...
                        _profileData.postValue(Success(oldProfileData))
                    },
                    {
