...
class SearchViewModel(
    ...
    private val repoResultStatus =
        MutableLiveData<Listing<Pair<Status, StatusViewData.Concrete>>>()
    val statuses: LiveData<PagedList<Pair<Status, StatusViewData.Concrete>>> =
        repoResultStatus.switchMap { it.pagedList }
    val networkStateStatus: LiveData<NetworkState> = repoResultStatus.switchMap {
        it.networkState
    }
    ...
    fun removeItem(status: Pair<Status, StatusViewData.Concrete>) {
        timelineCases.delete(status.first.id).subscribe({
            if (loadedStatuses.remove(status)) repoResultStatus.value?.refresh?.invoke()
        }, { err ->
    ...
    fun expandedChange(status: Pair<Status, StatusViewData.Concrete>, expanded: Boolean) {
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second).setIsExpanded(expanded).createStatusViewData()
            )
    ...
    fun reblog(status: Pair<Status, StatusViewData.Concrete>, reblog: Boolean) {
        timelineCases.reblog(status.first, reblog).observeOn(AndroidSchedulers.mainThread())
            .subscribe(
    ...
    private fun setRebloggedForStatus(
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second).setReblogged(reblog).createStatusViewData()
            )
            loadedStatuses[idx] = newPair
            repoResultStatus.value?.refresh?.invoke()
        }
    ...
    fun contentHiddenChange(status: Pair<Status, StatusViewData.Concrete>, isShowing: Boolean) {
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second).setIsShowingSensitiveContent(isShowing)
    ...
    fun collapsedChange(status: Pair<Status, StatusViewData.Concrete>, collapsed: Boolean) {
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second)
                    .setCollapsed(collapsed).createStatusViewData()
            )
            loadedStatuses[idx] = newPair
            repoResultStatus.value?.refresh?.invoke()
        }
    ...
    fun favorite(status: Pair<Status, StatusViewData.Concrete>, isFavorited: Boolean) {
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second).setFavourited(isFavorited)
                    .createStatusViewData()
            )
            loadedStatuses[idx] = newPair
            repoResultStatus.value?.refresh?.invoke()
        }
        timelineCases.favourite(status.first, isFavorited).onErrorReturnItem(status.first)
    ...
    fun bookmark(status: Pair<Status, StatusViewData.Concrete>, isBookmarked: Boolean) {
        ...
        if (idx >= 0) {
            val newPair = Pair(
                status.first,
                StatusViewData.Builder(status.second).setBookmarked(isBookmarked)
                    .createStatusViewData()
            )
            loadedStatuses[idx] = newPair
            repoResultStatus.value?.refresh?.invoke()
        }
        timelineCases.bookmark(status.first, isBookmarked).onErrorReturnItem(status.first)
            .subscribe().autoDispose()
    }
    ...
    fun emojiReact(react: Boolean, emoji: String, statusId: String) {
        loadedStatuses.indexOfFirst { it.first.id == statusId }.let { idx ->
            timelineCases.react(emoji, statusId, react)
                .subscribe({ newStatus -> setEmojiReactionForStatus(idx, newStatus) }, {
                    Log.d(
                        TAG,
                        "Failed to react with $emoji to ${loadedStatuses[idx].first.id}",
                        it
                    )
                }).autoDispose()
        }
